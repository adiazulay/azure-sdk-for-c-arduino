...
#define MQTT_DO_NOT_RETAIN_MSG  false

/* --- Time and NTP Settings --- */
#define NTP_SERVERS "time.nist.gov"
...
{
  int result;
  portenta_mqtt_client_config mqtt_config;
...
//   mqttClient.connect(mqtt_config.uri, mqtt_config.port);
  ...
  {
    int start_result = mqttClient.connect(mqtt_config.uri, mqtt_config.port);
...
//   int packet_id = esp_mqtt_client_subscribe((esp_mqtt_client_handle_t)mqtt_client_handle, (const char*)az_span_ptr(topic), (int)qos);
  int packet_id = mqttClient.subscribe((const char*)az_span_ptr(topic), (int)qos);
  return packet_id;
}
...
//   sntp_init();
//   sntp_setserver(NTP_SERVERS);

  ntpUdp.begin(localPort);
//   sendNTPpacket(NTP_SERVERS); // send an NTP packet to a time server
//   delay(1000);
...
//   configTime(GMT_OFFSET_SECS, GMT_OFFSET_SECS_DST, NTP_SERVERS);
  ...
  while (now_t < UNIX_TIME_NOV_13_2017)
  {
    delay(500);
    Serial.print(",");
    now_t = time(NULL);
...
time_t getNtpTime()
{
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      ...
      return secsSince1900 - 2208988800UL + PST_TIME_ZONE * SECS_PER_HOUR;
    }
  }
